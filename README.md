...
* Name: Required, 1-30 characters in length, any allowed.  This should include First/Middle/Last names as applicable
...
* Hair: Required, 3-10 characters in length, alpha only.  Use "MULTI" for now for multicolored hair
* Offense: Required, 5-15 characters in length, any allowed
* Date of Warrant/Violation: Required, 8 characters in length, numeric in MMDDYYYY format (allows dates from 1900 to today +1 day, to account for time zone differences).  When both dates are available, use the Date of Warrant
...
The submit button will submit the fields for validation and generate a message or errors.  The clear button will reset all fields to blank.

### Change Report
* Whenever all fields are clear, the submit button is disabled.
* Name requirements updated for clarity.
* Hair requirements updated for clarity.
* The undo button has been replaced by the clear button.
* Fixed the "Key" error that appeared in the console
...
* Unknown handling: For limited fields, they can be unknown and still be valid entries.
* Placeholder text to indicate required entries in a field.
* Required field indications.
